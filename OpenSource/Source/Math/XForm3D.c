...
geXForm3d_TransformArray(const geXForm3d *XForm, const geVec3d *Source, geVec3d *Dest, int32 Count)
{
	#define FSIZE	4
...
*/
	for (int i = 0; i < Count; i++) {
		// Matrix multiplication with translation
		// Calculate X component
		Dest[i].X = Source[i].X * XForm->AX + 
					Source[i].Y * XForm->BX + 
					Source[i].Z * XForm->CX + 
					XForm->Translation.X;
		
		// Calculate Y component
		Dest[i].Y = Source[i].X * XForm->AY + 
					Source[i].Y * XForm->BY + 
					Source[i].Z * XForm->CY + 
					XForm->Translation.Y;
		
		// Calculate Z component
		Dest[i].Z = Source[i].X * XForm->AZ + 
					Source[i].Y * XForm->BZ + 
					Source[i].Z * XForm->CZ + 
					XForm->Translation.Z;
	}

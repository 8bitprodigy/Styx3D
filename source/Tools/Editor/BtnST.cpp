...
/*
	// Note: the following two lines MUST be here! even if
	// BoundChecker says they are unnecessary!
	if (m_hIconIn != nullptr) ::DestroyIcon(m_hIconIn);
	if (m_hIconOut != nullptr) ::DestroyIcon(m_hIconOut);
...
void CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	m_hIconIn = hIconIn;
	// Set icon when the mouse is OUT the button
	m_hIconOut = (hIconOut == nullptr) ? m_hIconIn : hIconOut;
...
BOOL CButtonST::SetBtnCursor(int nCursorId)
{
	...
	if (m_hCursor != nullptr) ::DestroyCursor(m_hCursor);
	m_hCursor = nullptr;
	...
	if (nCursorId != -1)
	{
		...
		if (m_hCursor == nullptr) return FALSE;
	}
